...
class TeamController(SatControllerInterface):
    ...
    def team_run(self, system_state: sat_msgs.SystemState, satellite_state: sat_msgs.SatelliteState, dead_sat_state: sat_msgs.SatelliteState) -> sat_msgs.ControlMessage:
        ...
        if sqrt(xdistance**2+ydistance**2)<docking: #DOCKING PART
            control_message.thrust.f_x = -40 * (satellite_state.pose.x - ((dead_sat_state.pose.x)+(0.25*cos(dead_sat_state.pose.theta - math.pi / 2)))) - 10 * satellite_state.twist.v_x
            control_message.thrust.f_y = -40 * (satellite_state.pose.y - ((dead_sat_state.pose.y)+(0.25*sin(dead_sat_state.pose.theta - math.pi / 2)))) - 10 * satellite_state.twist.v_y
            control_message.thrust.tau = -50 * (satellite_state.pose.theta - ((dead_sat_state.pose.theta)+math.pi)) - 18.0 * satellite_state.twist.omega
        elif sqrt(xdistance**2+ydistance**2)<radius: #shpere# Set thrust command values, basic PD controller that drives the sat to [0, -1]
            control_message.thrust.f_x = -8 * (satellite_state.pose.x - ((dead_sat_state.pose.x)+(0.25*cos(dead_sat_state.pose.theta - math.pi / 2)))) - 3 * satellite_state.twist.v_x
            control_message.thrust.f_y = -8 * (satellite_state.pose.y - ((dead_sat_state.pose.y)+(0.25*sin(dead_sat_state.pose.theta - math.pi / 2)))) - 3 * satellite_state.twist.v_y
            if sqrt(xdistance**2+ydistance**2)<0.4:
                control_message.thrust.tau = -40 * (satellite_state.pose.theta - ((dead_sat_state.pose.theta)+math.pi)) - 20.0 * satellite_state.twist.omega    
            else:    
            ...
            control_message.thrust.f_x = -4 * (satellite_state.pose.x - ((dead_sat_state.pose.x)+(0.44*cos(dead_sat_state.pose.theta - math.pi / 2)))) - 5 * satellite_state.twist.v_x
            control_message.thrust.f_y = -4 * (satellite_state.pose.y - ((dead_sat_state.pose.y)+(0.44*sin(dead_sat_state.pose.theta - math.pi / 2)))) - 5 * satellite_state.twist.v_y
            if sqrt(xdistance**2+ydistance**2)<1:
                control_message.thrust.tau = -30 * (satellite_state.pose.theta - ((dead_sat_state.pose.theta)+math.pi)) - 15.0 * satellite_state.twist.omega    
